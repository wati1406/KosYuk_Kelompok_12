...
$active_tab = isset($_GET['tab']) ? $_GET['tab'] : 'data-diri';
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Kwitansi Print Styles */
        @media print {
            body * {
                visibility: hidden;
            }
            .kwitansi-print, .kwitansi-print * {
                visibility: visible;
            }
            .kwitansi-print {
                position: absolute;
        ...
        .kwitansi-detail-item {
            margin-bottom: 15px;
            padding-bottom: 15px;
            border-bottom: 1px solid #eee;
        }
        
        .kwitansi-detail-label {
            font-weight: 600;
            color: #0A2C4F;
            margin-bottom: 5px;
        }
        
        .kwitansi-detail-value {
            color: #333;
        }
        ...
        .kwitansi-simple-container {
            padding: 30px;
            font-family: 'Courier New', monospace;
        }
        
        .kwitansi-simple-item {
            margin-bottom: 12px;
            line-height: 1.5;
        }
        
        .kwitansi-simple-label {
            font-weight: bold;
            margin-right: 10px;
        }
    </style>
...
<body>
    ...
    <div class="profile-wrapper">
        ...
        <div class="profile-sidebar">
            ...
            <nav class="profile-nav">
                <a href="?tab=data-diri" class="nav-item <?php echo $active_tab == 'data-diri' ? 'active' : ''; ?>" data-tab="data-diri">
                    <i class="fas fa-user"></i> Data Diri
                </a>
                <a href="?tab=favorit" class="nav-item <?php echo $active_tab == 'favorit' ? 'active' : ''; ?>" data-tab="favorit">
                    <i class="fas fa-heart"></i> Kos Favorit
                </a>
                <a href="?tab=booking-saya" class="nav-item <?php echo $active_tab == 'booking-saya' ? 'active' : ''; ?>" data-tab="booking-saya">
                    <i class="fas fa-calendar-check"></i> Booking 
                </a>
                <a href="?tab=keamanan" class="nav-item <?php echo $active_tab == 'keamanan' ? 'active' : ''; ?>" data-tab="keamanan">
        ...
        <div class="profile-content-wrapper">
            <div class="profile-content">
                ...
                <!-- Tab: Data Diri -->
                <div id="data-diri" class="tab-content <?php echo $active_tab == 'data-diri' ? 'active' : ''; ?>">
                ...
                <!-- Tab: Kos Favorit -->
                <div id="favorit" class="tab-content <?php echo $active_tab == 'favorit' ? 'active' : ''; ?>">
                    <div class="content-header">
                        <h1>Kos Favorit</h1>
                ...
                <!-- Tab: Booking Saya -->
                <div id="booking-saya" class="tab-content <?php echo $active_tab == 'booking-saya' ? 'active' : ''; ?>">
                    <div class="content-header">
                        <h1>Booking </h1>
                    </div>
                    ...
                    <?php else: 
                        ...
                        // Jika booking_data bukan array (single booking), ubah ke array
                        if (isset($booking_data['id_kos'])) {
                            $booking_data = [$booking_data];
                        }
                    ?>
                    <div class="booking-list">
                        <?php foreach($booking_data as $index => $booking): 
                            // Pastikan data user ada di booking
                            if (!isset($booking['user_nama'])) {
                                $booking['user_nama'] = $user_data['nama_lengkap'];
                                $booking['user_email'] = $user_data['email'];
                            }
                            
                            // Hitung total biaya
                            $harga_per_tahun = $booking['harga_kos'];
                            $total_biaya = $harga_per_tahun;
                            
                            if ($booking['durasi_sewa'] == '6bulan') {
                                $total_biaya = $harga_per_tahun / 2;
                                $durasi_label = '6 Bulan';
                                $harga_per_bulan = $total_biaya / 6;
                            } elseif ($booking['durasi_sewa'] == '1tahun') {
                                $durasi_label = '1 Tahun';
                                $harga_per_bulan = $total_biaya / 12;
                            } else {
                                $total_biaya = $harga_per_tahun * 2 * 0.95; // Diskon 5% untuk 2 tahun
                                $durasi_label = '2 Tahun';
                                $harga_per_bulan = $total_biaya / 24;
                            }
                            
                            if ($booking['jenis_pembayaran'] == 'dp') {
                                $dp_amount = 1000000;
                                $sisa_pembayaran = $total_biaya - $dp_amount;
                            }
                        ?>
                        <div class="booking-card" id="booking-<?php echo $index; ?>">
                            ...
                            <div class="booking-details">
                                <p><strong>Nama Pemesan:</strong> <?php echo htmlspecialchars($booking['user_nama']); ?></p>
                                ...
                                <p><strong>Harga Kos per Tahun:</strong> Rp <?php echo number_format($harga_per_tahun, 0, ',', '.'); ?></p>
                                <p><strong>Jenis Pembayaran:</strong> <?php echo $booking['jenis_pembayaran'] == 'dp' ? 'Down Payment (DP)' : 'Lunas'; ?></p>
                                <p><strong>Durasi Sewa:</strong> <?php echo $durasi_label; ?></p>
                                <p><strong>Harga per Bulan:</strong> Rp <?php echo number_format($harga_per_bulan, 0, ',', '.'); ?></p>
                                <p><strong>Total Biaya:</strong> Rp <?php echo number_format($total_biaya, 0, ',', '.'); ?></p>
                                <?php if($booking['jenis_pembayaran'] == 'dp'): ?>
                                <p><strong>Down Payment:</strong> Rp <?php echo number_format($dp_amount, 0, ',', '.'); ?></p>
                                <p><strong>Sisa Pembayaran:</strong> Rp <?php echo number_format($sisa_pembayaran, 0, ',', '.'); ?></p>
                                <?php endif; ?>
                            ...
                            <div class="booking-actions">
                                <button class="btn-booking btn-booking-primary" onclick="showKwitansiModal(<?php echo $index; ?>)">
                ...
                <!-- Tab: Keamanan -->
                <div id="keamanan" class="tab-content <?php echo $active_tab == 'keamanan' ? 'active' : ''; ?>">
                    <div class="content-header">
                        <h1>Keamanan Akun</h1>
                    </div>
    ...
    <!-- Kwitansi Modal -->
    <div id="kwitansiModal" class="kwitansi-modal">
        <div class="kwitansi-content">
            <div class="kwitansi-header">
                <h2>Kwitansi Booking</h2>
                <button type="button" class="btn-close" onclick="closeKwitansiModal()"></button>
            </div>
            <div class="kwitansi-body" id="kwitansiBody">
                <!-- Kwitansi content will be loaded here -->
            </div>
            <div class="kwitansi-footer">
                <button type="button" class="btn btn-secondary" onclick="closeKwitansiModal()">
                    <i class="fas fa-times"></i> Tutup
                </button>
                <button type="button" class="btn btn-primary" onclick="printKwitansiContent()">
    ...
    function switchTab(tabId) {
        ...
        // Update URL tanpa reload halaman (untuk UX)
        history.replaceState(null, null, '?tab=' + tabId);
    }
    
    // Inisialisasi tab saat halaman dimuat
    document.addEventListener('DOMContentLoaded', function() {
        // Set default tab dari URL parameter atau cookie/localStorage
    ...
    // Kwitansi Functions
    function showKwitansiModal(index) {
        if (bookingData[index]) {
            const booking = bookingData[index];
            
            // Pastikan data user ada di booking
            if (!booking.user_nama) {
                booking.user_nama = userData.nama_lengkap;
                booking.user_email = userData.email;
            }
            
            // Hitung detail harga
            let harga_per_tahun = booking.harga_kos;
            let total_biaya = harga_per_tahun;
            let durasi_label = '1 Tahun';
            let harga_per_bulan = total_biaya / 12;
            
            if (booking.durasi_sewa === '6bulan') {
                total_biaya = harga_per_tahun / 2;
                durasi_label = '6 Bulan';
                harga_per_bulan = total_biaya / 6;
            } else if (booking.durasi_sewa === '2tahun') {
                total_biaya = harga_per_tahun * 2 * 0.95; // Diskon 5%
                durasi_label = '2 Tahun';
                harga_per_bulan = total_biaya / 24;
            }
            ...
            const kwitansiHTML = `
                <div class="kwitansi-print">
                    <div class="kwitansi-simple-container">
                        <h2 style="text-align: center; color: #0A2C4F; margin-bottom: 30px;">Kwitansi Booking</h2>
                        
                        <div style="margin-bottom: 30px;">
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Nama Pemesan:</span> ${booking.user_nama}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Kode Booking:</span> ${booking.kode_booking}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Alamat:</span> ${booking.alamat_kos}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Harga Kos per Tahun:</span> ${formatRupiah(harga_per_tahun)}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Jenis Pembayaran:</span> ${booking.jenis_pembayaran == 'dp' ? 'Down Payment (DP)' : 'Lunas'}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Durasi Sewa:</span> ${durasi_label}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Harga per Bulan:</span> ${formatRupiah(harga_per_bulan)}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Total Biaya:</span> ${formatRupiah(total_biaya)}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Tanggal Masuk:</span> ${formatDate(booking.tanggal_masuk)}
                            </div>
                            <div class="kwitansi-simple-item">
                                <span class="kwitansi-simple-label">Tanggal Booking:</span> ${formatDateTime(booking.tanggal_booking)}
                            </div>
                        </div>
                        
                        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd;">
                            <p style="color: #666; font-size: 0.9rem;">Terima kasih telah menggunakan layanan KosYuk</p>
                        </div>
                    </div>
                </div>
            `;
            
            // Tampilkan kwitansi di modal
            document.getElementById('kwitansiBody').innerHTML = kwitansiHTML;
    ...
    function printKwitansiContent() {
        const printContent = document.querySelector('.kwitansi-print').innerHTML;
        ...
        // Refresh halaman untuk mengembalikan event listeners
        location.reload();
    }
    
    // Batalkan booking
    function batalkanBooking(index) {
    ...
    // Fungsi untuk memastikan tab sesuai dengan URL saat dimuat
    (function() {
